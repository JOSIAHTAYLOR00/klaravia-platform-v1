...
exports.handler = async (event) => {
    ...
    const getProposeData = async () => {
        try {
        
            const config = {
                method: 'get',
                url: `https://apicenter.eagleview.com/solar/v1/solarready/data?trackingId=${trackingId}&pointData=TRUE&monthlyProductionData=TRUE&productId=95`,
                headers: { 
            ...
            return {
                ...
                body: JSON.stringify({
                    message: 'Error getting the propose data set',
                    error: error.message
                })
            };
        }
    ...
    const proposeData = await getProposeData().then((res) => res.body[0]);
    ...
    const getPanelLayoutImage = async () => {
        try {
            ...
            const config = {
                method: 'get',
                url: `https://apicenter.eagleview.com/solar/v1/solarready/image?trackingId=${trackingId}&facetLabel=FALSE&TSRFHeatmap=FALSE&panel=TRUE&fileFormat=PNG&highResolution=TRUE`,
                headers: { 
                    'Content-Type': 'application/json',
