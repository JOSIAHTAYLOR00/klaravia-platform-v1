...
export function calculateBaseSystemSize(userData: any, CONSTANTS: SolarConstants) {
  const annualConsumption = Number(userData.estimatedAnnualUsage);
  return (annualConsumption / 
    (CONSTANTS.avgSunHoursPerDay * 365 * CONSTANTS.systemEfficiency)) * 0.9;
}

export function adjustPanels(
  prevAdjustment: number,
  adjustment: number,
  baseSystemSize: number,
  CONSTANTS: SolarConstants
) {
  const newAdjustment = prevAdjustment + adjustment;
...
export function getTotalAnnualProductionByMonth(data: any) {
  const totalMonthlyProduction = new Array(12).fill(0);

  if (data) {
    data.energyTable.forEach((panel: any) => {
      panel["Panels Monthly Production"].forEach((monthlyProduction: number, index: number) => {
...
export async function calculateSystemEstimates(
  ...
  try {
    ...
    const baseSystemSize = calculateBaseSystemSize(userData, CONSTANTS);
    const basePanels = Math.ceil((baseSystemSize * 1000) / CONSTANTS.panelWattage);
